...
const cfgPath = `${process.cwd()}/data/cos_whitelist.json`
function readWhitelist() {
    try { return JSON.parse(fs.readFileSync(cfgPath, 'utf8')) } catch { return [] }
}
function hasPermission(e) {
    if (e.isMaster) return true
    let wl = readWhitelist()
    let uid = String(e.user_id), gid = e.isGroup ? String(e.group_id) : ''
    for (const item of wl) {
        if (item.startsWith('^') && item.slice(1) === uid) return true
        if (item.includes('^') && e.isGroup) {
            let [g, u] = item.split('^')
            if (g === gid && u === uid) return true
        }
        if (e.isGroup && item === gid) return true
    }
    return false
}
