...
const HELP_FALLBACK_LINES = [
  ...
  '  #下载咕咕牛: 下载图库',
  '  #更新咕咕牛: 手动更新',
  '  #咕咕牛设置加速前缀[URL/关闭]: 设置 Git 仓库加速前缀',
  '',
  '【图库操作】',
  '  #启/禁用咕咕牛: 管理同步',
  '  #咕咕牛状态: 查看状态',
...
class Ananke {
    ...
    static #FilterCfg(data) {
        const allowedKeys = ['Repo_Ops', 'PFL_Ops', 'RenderScale', 'Ai', 'EasterEgg', 'layout', 'SR18_Ops', 'GitProxyPrefix', 'GitRetryCount', 'GitRetryDelay', 'GitByteIdleTimeout'];
...
class Nomos {
    ...
    static getResPoolRemoteUrl(config = MiaoPluginMBT.MBTConfig) {
        return MiaoPluginMBT.BuildGitRepoUrl(this.#_crppRepoUrl, config);
    }

    static async syncResPoolRemote(logger = console, config = MiaoPluginMBT.MBTConfig) {
        const Hades = getHades(logger);
        if (!this.hasResPoolLocal()) return false;
        const remoteUrl = this.getResPoolRemoteUrl(config);
        if (!remoteUrl) return false;
        try {
            await MBTPipeControl('git', ['remote', 'set-url', 'origin', remoteUrl], { cwd: this.ResPoolDir, Rid: 'CRPP', caDisabled: true, gitConfigs: ['gc.auto=0'] }, 10000);
            Hades.D(`前置资源仓库 origin 已同步为当前加速配置。`);
            return true;
        } catch (err) {
            Hades.W(`同步前置资源仓库 origin 失败: ${err.message}`);
            return false;
        }
    }
    ...
    static async syncResPool(options = {}) {
        ...
        const doClone = async (signal) => {
            ...
            await MBTPipeControl(
                'git',['clone', '--template=', '--depth=1', '--single-branch', '-b', 'master', this.getResPoolRemoteUrl(), targetDir],
        ...
        try {
            await this.ResPoolLock.run(async (signal) => {
                ...
                const syncByReset = async () => {
                    ...
                    await this.syncResPoolRemote(Hades);
...
class MiaoPluginMBT extends plugin {
  ...
  static ToImgSeg(input, options = {}) {
    ...
    const imageBuffer = toBuffer(input);
    if (imageBuffer) {
      return segment.image(toBase64Url(imageBuffer));
    }
  ...
  static NormalizeGitProxyPrefix(raw = "") {
    const value = String(raw ?? "").trim();
    if (!value || /^(关闭|清空|直连|无|off|none|clear)$/i.test(value)) return "";
    const prefix = value.replace(/\/+$/, "");
    if (!/^https?:\/\//i.test(prefix)) {
      throw new Error("加速前缀必须以 http:// 或 https:// 开头，或使用“关闭”");
    }
    if (/^https?:\/\/github\.com\/?$/i.test(prefix)) return "";
    return prefix;
  }

  static GitProxyPrefix(config = MiaoPluginMBT.MBTConfig) {
    return MiaoPluginMBT.NormalizeGitProxyPrefix(config?.GitProxyPrefix ?? DFC.GitProxyPrefix);
  }

  static BuildGitRepoUrl(repoUrl, config = MiaoPluginMBT.MBTConfig) {
    const cleanRepo = String(repoUrl || "").trim().replace(/\/+$/, "");
    if (!cleanRepo) return "";
    const gitRepo = cleanRepo.endsWith(".git") ? cleanRepo : `${cleanRepo}.git`;
    const prefix = MiaoPluginMBT.GitProxyPrefix(config);
    if (!prefix) return gitRepo;

    const repoPath = gitRepo.replace(/^https?:\/\/github\.com\//i, "");
    const githubPath = repoPath.startsWith("github.com/") ? repoPath : `github.com/${repoPath}`;
    if (!/[{](?:url|repo|path)[}]/.test(prefix)) return `${prefix}/${gitRepo}`;
    return prefix
      .replace(/\{url\}/g, gitRepo)
      .replace(/\{repo\}/g, repoPath)
      .replace(/\{path\}/g, githubPath);
  }

  static GitNodeName(config = MiaoPluginMBT.MBTConfig) {
    return MiaoPluginMBT.GitProxyPrefix(config) ? "用户加速前缀" : "GitHub";
  }
  ...
  static async BuildSetPanelHtml(tplFile, logger = getCore()) {
      const Hades = getHades(logger);
      try {
          let html = await Ananke.readFile(tplFile, 'utf-8');
          if (!html) return null;
          if (html.includes('Git加速前缀') && html.includes('{{ GitProxyPrefix.text }}')) return html;

          const htmlBlock = `          <div class="setting-item">
            <div class="item-left">
              <span class="label">Git加速前缀</span>
              <div class="value-description">下载、更新与前置资源仓库使用的 Git 地址前缀</div>
              <div class="command-example">#咕咕牛设置加速前缀[URL/关闭]</div>
            </div>
            <div class="item-right">
              <span class="value {{ GitProxyPrefix.class }}">{{ GitProxyPrefix.text }}</span>
            </div>
          </div>
`;

          if (!html.includes('{{ GitProxyPrefix.text }}')) {
              html = html.replace('        </div>\n      </main>', `${htmlBlock}        </div>\n      </main>`);
          }

          return html;
      } catch (err) {
          Hades.W(`设置面板加速前缀模块注入失败: ${err.message}`);
          return null;
      }
  }
  ...
  static async SmartTaskHeavy(runtimeContext, repoNum, repoUrl, branch, finalLocalPath, e, logger, sortedNodes = [], MBTProcc, signal = null, lockId = null, Cer_SessionId = null) {
      ...
      try {
          proxyPrefix = MiaoPluginMBT.GitProxyPrefix();
          actualCloneUrl = MiaoPluginMBT.BuildGitRepoUrl(cleanRepoUrl);
          nodeName = MiaoPluginMBT.GitNodeName();
          MODE = proxyPrefix ? 'USER_GIT_PREFIX' : 'GITHUB_DIRECT';
          logModeMsg = proxyPrefix ? proxyPrefix : 'GitHub直连';
          const uniqueSuffix = crypto.randomBytes(4).toString('hex');
          const tempRepoPath = path.join(MiaoPluginMBT.Paths.TempDownloadPath, `Git-${repoNum}-${uniqueSuffix}`);
          Hades.O(`${RidColored} ${logTag} | 正在下载 [${nodeName}]`);
          const retryCount = Math.max(1, Number(MiaoPluginMBT.MBTConfig?.GitRetryCount ?? DFC.GitRetryCount) || DFC.GitRetryCount);
          const retryDelay = Math.max(1000, Number(MiaoPluginMBT.MBTConfig?.GitRetryDelay ?? DFC.GitRetryDelay) || DFC.GitRetryDelay);
          const byteIdleTimeout = Math.max(90000, Number(MiaoPluginMBT.MBTConfig?.GitByteIdleTimeout ?? DFC.GitByteIdleTimeout) || DFC.GitByteIdleTimeout);

          await MiaoPluginMBT.GitMutex.run(async () => {
              if (signal?.aborted) throw new Error('IO前中止');
          ...
          const gitOptions = {
              cwd: MiaoPluginMBT.Paths.YzPath,
              shell: false,
              Rid,
              RidTag: RidColored,
              inheritEnv: true,
              gitConfigs: ['http.sslVerify=false', 'gc.auto=0'],
              caWhitelist: [actualCloneUrl, cleanRepoUrl].filter(Boolean),
              caDisabled: true,
              disableTurtleCheck: true,
              constraints: {
                  byteidleThreshold: byteIdleTimeout,
                  zombieThreshold: byteIdleTimeout + 60000,
                  hardTimeout: Math.max(DFC.GitTimeout, byteIdleTimeout * 2)
              },
              env: sysProxyEnv
          };
          if (signal) gitOptions.signal = signal;

          let lastGitErr = null;
          for (let attempt = 1; attempt <= retryCount; attempt++) {
              if (signal?.aborted) throw new Error(`下载中止: ${signal.reason}`);

              const hasGitDir = await Ananke.Audit(path.join(tempRepoPath, ".git"), false);
              const useResume = hasGitDir && attempt > 1;
              const runOpts = useResume ? { ...gitOptions, cwd: tempRepoPath } : gitOptions;
              const runArgs = useResume
                  ? ["fetch", "--progress", `--depth=${DFC.Depth}`, "origin", targetBranch]
                  : cloneArgs;

              try {
                  if (useResume) {
                      Hades.W(`${RidColored} ${logTag} | 上次下载中断，尝试续传补齐 (${attempt}/${retryCount})`);
                      await MBTPipeControl("git", ["remote", "set-url", "origin", actualCloneUrl], runOpts, 10000, null, null, null, null, MBTProcc).catch(() => {});
                  } else if (attempt > 1) {
                      Hades.W(`${RidColored} ${logTag} | 重新下载 (${attempt}/${retryCount})`);
                      await Ananke.obliterate(tempRepoPath);
                  }

                  await MBTPipeControl("git", runArgs, runOpts, DFC.GitTimeout, null, null, null, null, MBTProcc);

                  if (useResume) {
                      await MBTPipeControl("git", ["checkout", "-B", targetBranch, "FETCH_HEAD"], runOpts, 30000, null, null, null, null, MBTProcc);
                      await MBTPipeControl("git", ["reset", "--hard", "FETCH_HEAD"], runOpts, 30000, null, null, null, null, MBTProcc);
                  }
                  lastGitErr = null;
                  break;
              } catch (err) {
                  ...
                  const backoff = retryDelay * attempt + MBTMath.Range(300, 1200);
                  Hades.W(`${RidColored} ${logTag} | 下载失败: ${err.message}，${(backoff / 1000).toFixed(1)}s 后重试`);
                  await common.sleep(backoff);
              }
          }

          if (lastGitErr) throw lastGitErr;

          if (signal?.aborted) throw new Error(`提交前中止: ${signal.reason}`);

          await MiaoPluginMBT.GitMutex.run(async () => {
              if (repoNum === 1 && !(await Ananke.Audit(path.join(tempRepoPath, "CowCoo/html")))) {
              ...
              await Ananke.rename(tempRepoPath, finalLocalPath);
          }, {
              id: commitLockId,
              instant: true
          });
          ...
          return { success: true, nodeName, error: null, gitLog, mode: MODE, modeMsg: logModeMsg };
      } catch (directErr) {
          Hades.E(`${RidColored} ${logTag} 下载失败:${directErr.message}`);
          if (Cer_SessionId) cerberus.finishSession(Cer_SessionId, false, { event: 'download-failed', code: directErr?.code, message: directErr?.message });
          return { success: false, nodeName, error: directErr, mode: MODE, modeMsg: logModeMsg };
      } finally {
  ...
  static async UpstreamSyncRepo(e, RepoNum, localPath, RepoName, RepoUrl, branch, isScheduled, logger, senseChain) {
    return await MiaoPluginMBT.GitMutex.run(async () => {
      ...
      const retryCount = Math.max(1, Number(cfg.GitRetryCount ?? DFC.GitRetryCount) || DFC.GitRetryCount);
      const retryDelay = Math.max(1000, Number(cfg.GitRetryDelay ?? DFC.GitRetryDelay) || DFC.GitRetryDelay);
      const byteIdleTimeout = Math.max(90000, Number(cfg.GitByteIdleTimeout ?? DFC.GitByteIdleTimeout) || DFC.GitByteIdleTimeout);
      ...
      const initial_pipe_opts = {
        inheritEnv: true,
        env: null,
        gitConfigs: ['gc.auto=0'],
        airlock: false,
        disableTurtleCheck: true,
        constraints: {
          byteidleThreshold: byteIdleTimeout,
          zombieThreshold: byteIdleTimeout + 60000,
          hardTimeout: Math.max(basePullTimeout, byteIdleTimeout * 2)
        }
      };
      ...
      let remoteUrl = "";
      try {
        remoteUrl = MiaoPluginMBT.BuildGitRepoUrl(RepoUrl, cfg);
      } catch (err) {
        state.error = err;
        return state;
      }
      if (remoteUrl) {
        await MBTPipeControl("git", ["remote", "set-url", "origin", remoteUrl], { cwd: localPath, ...initial_pipe_opts }).catch((err) => {
          Hades.W(`${RepoName} 设置远端失败: ${err.message}`);
        });
      }

      Hades.D(`${RepoName} 开始更新，模式=${MiaoPluginMBT.GitNodeName(cfg)}`);
      let syncResult = null;
      for (let attempt = 1; attempt <= retryCount; attempt++) {
        ...
        const isRetryable = !!syncResult.error && (
          GIT_TIMEOUT_ERR_CODES.has(String(syncResult.error.code).toUpperCase())
          || GIT_NETWORK_ERR_KEYWORDS.some(k => syncErrText.includes(k))
        );
        if (!isRetryable || attempt >= retryCount) break;

        const backoff = retryDelay * attempt + MBTMath.Range(300, 1200);
        Hades.W(`${RepoName} 更新失败: ${syncResult.error.message}，${(backoff / 1000).toFixed(1)}s 后重试 (${attempt + 1}/${retryCount})`);
        await common.sleep(backoff);
      }

      if (!syncResult.success) {
        Hades.E(`${RepoName} 更新失败，已按当前配置直接结束。`);
      }
...
static async ProvisionPhase(e, logger = getCore(), stage = 'full') {
 ...
 static async HydrateCore(e, isScheduled = false, logger = getCore()) {
    ...
    try {
      ...
      const deployRepo = async (repoTask) => {
          ...
          const result = await MiaoPluginMBT.SmartTaskHeavy(
              ...
              repoPath, e, Hades, [], MBTProcc, null, null, sessionId
          );
  ...
  async Reconcile(e, isScheduled = false) {
    ...
    reportResults.push(await deployRepoResult(1, MiaoPluginMBT.Paths.MountRepoPath, Repo1, "Main_Github_URL", "Main_Github_URL", branch, true, null));

    if (activeRepoIds.has(2)) {
      const repo2Status = Repo2Exists
        ? await deployRepoResult(2, MiaoPluginMBT.Paths.MountRepoPath2, Repo2, "Ass_Github_URL", "Ass_Github_URL", branch, false, null)
    ...
    if (activeRepoIds.has(3)) {
      const repo3Status = Repo3Exists
        ? await deployRepoResult(3, MiaoPluginMBT.Paths.MountRepoPath3, Repo3, "Ass2_Github_URL", "Ass2_Github_URL", branch, false, null)
    ...
    if (activeRepoIds.has(4)) {
      const repo4Status = Repo4Exists
        ? await deployRepoResult(4, MiaoPluginMBT.Paths.MountRepoPath4, Repo4, "Ass3_Github_URL", "Ass3_Github_URL", branch, false, null)
  ...
  async MBTOpsDeck(e, StatusMsg = "") {
    ...
    try {
        const ViewProps = {
            ...
            GitProxyPrefix: (() => {
                const value = MiaoPluginMBT.GitProxyPrefix(config);
                return {
                    raw: value,
                    text: value || 'GitHub直连',
        ...
        const tplFile = path.join(MiaoPluginMBT.Paths.OpsPath, "html", "tools", "setpanel.html");
        const htmlContent = await MiaoPluginMBT.BuildSetPanelHtml(tplFile, logger);

        const imgBuffer = await Morpheus.shot("Settings", {
            ...(htmlContent ? { htmlContent } : { tplFile }),
            data: ViewProps,
            logger: logger,
            pageBoundingRect: { selector: ".container" }
        });
  ...
  async RouteOpsHub(e) {
    ...
    const proxyMatch = msg.match(/^#咕咕牛设置(?:git|Git|GitHub|github)?加速前缀\s*(.+)$/i);
    if (proxyMatch) {
        return this.MutateGitProxyPrefix(e, proxyMatch[1]);
    }
  ...
  async MutateGitProxyPrefix(e, rawValue) {
    const logger = getHades(this.logger);
    let parsedVal;
    try {
        parsedVal = MiaoPluginMBT.NormalizeGitProxyPrefix(rawValue);
    } catch (err) {
    ...
    try {
        ...
        const currentVal = MiaoPluginMBT.NormalizeGitProxyPrefix(config.GitProxyPrefix ?? DFC.GitProxyPrefix);
        if (currentVal === parsedVal) {
            return Pheme.quote(e, `Git加速前缀已经是设定值啦。\n当前: ${parsedVal || 'GitHub直连'}`);
        }

        config.GitProxyPrefix = parsedVal;
        const saved = await Ananke.SaveConfig(MiaoPluginMBT.Paths.ConfigFilePath, config, logger);
        if (!saved) return Pheme.quote(e, "Git加速前缀保存失败，请查看后台日志。");

        MiaoPluginMBT.MBTConfig = config;
        await Nomos.syncResPoolRemote(logger, config);
        return Pheme.quote(e, `Git加速前缀已更新。\n当前: ${parsedVal || 'GitHub直连'}`);
    } catch (err) {
        logger.E(`Git加速前缀设置失败:`, err);
        return Pheme.quote(e, `Git加速前缀设置失败: ${err.message}`);
    }
  }
  ...
  async MutateSubState(e, key, valRaw) {
    ...
    const strategies = {
        ...
        '加速前缀': {
            cfgKey: 'GitProxyPrefix',
            name: 'Git加速前缀',
            parse: (v) => MiaoPluginMBT.NormalizeGitProxyPrefix(v),
            sideEffect: async () => {
                await Nomos.syncResPoolRemote(logger, MiaoPluginMBT.MBTConfig);
            }
        },
        'sr18': {
            cfgKey: 'SR18_Ops',
    ...
    if (key === '加速前缀' && !(await MiaoPluginMBT.ICI())) {
        await Pheme.quote(e, mutation.msg);
        return true;
    }
...
const CowCoo_Rules = [
  ...
  [/^#咕咕牛查看\s*.*$/i, "QueryData"],
  [/^#咕咕牛帮助$/i, "Help"],
  [/^#咕咕牛设置(?:git|Git|GitHub|github)?加速前缀\s*.+$/i, "RouteOpsHub", "master"],
